<?php

const SITE_NAME = 'Template 6';
const SITE_CODE_NAME = 'template-6';
const SITE_URL = 'http://template6.test/';
const SITE_ADMIN_URL = 'http://template6.test/admin/';
const SUPPORT_EMAIL = 'mail@email.com';
const API_URL = 'http://route.test/api';
const VERSION = '1.0.0';

// contact.php
const TXT_CONTACT_TITLE = '';
const URL_MAP = '';
const TXT_BUTTON_SEND = '';
const NUMBER_OF_CONTACTS = '';
const EMAIL_INFO = '';
const EMAIL_CONTACT = '';
const LOCATE_CONTACT = '';

// footer.php
const COPYRIGHT = '';
const TXT_COPYRIGHT_DESC = '';
CONST TXT_COPYRIGHT_DISTRIBUTION = '';

// headerArea.php & headerSticky.php
const LOGO = '';
const SCROLL_TOP = '';
const SCROLL_SERVICES = '';
const SCROLL_PROJECTS = '';
const PAGES = '';
CONST ABOUT = ''; 
CONST FAQS = '';
const SCROLL_TO_INFOS = '';
const SCROLL_TO_CONTACT = '';
const MENU_TRIGGER = '';

// infos.php
const IMG_INFOS = '';
const TXT_INFOS_TITLE = '';
const TXT_INFO = '';
const STATISTICS_ONE = '';
const TXT_STATISTICS_TWO = '';
const STATISTICS_TWO = '';
const TXT_STATISTICS_THREE = '';
const STATISTICS_THREE = '';
const TXT_STATISTICS_DESC = '';

//mainBanner.php
const TXT_BANNER_TITLE = '';
const TXT_BANNER_SUBTITLE = '';    
const TXT_BANNER_BUTTON = '';
const ALTERNATIVE_BANNER = '';
const TXT_BANNER_SECOND_BUTTON = '';

//preHeader.php
const FA_PHONE = '';
const FA_EMAIL = '';
const FA_MAP_MARKER = '';

//projects.php
const TXT_PROJECTS_TITLE = '';
const TXT_PROJECTS_DESC = '';
const IMG_PROJECTS_ITEM_ONE = '';
const TXT_PROJECTS_ITEM_ONE = '';
const IMG_PROJECTS_ITEM_TWO = '';
const TXT_PROJECTS_ITEM_TWO = '';
const IMG_PROJECTS_ITEM_THREE = '';
const TXT_PROJECTS_ITEM_THREE = '';
const IMG_PROJECTS_ITEM_FOUR = '';
const TXT_PROJECTS_ITEM_FOUR = '';
const IMG_PROJECTS_ITEM_FIVE = '';
const TXT_PROJECTS_ITEM_FIVE = '';
const IMG_PROJECTS_ITEM_SIX = '';
const TXT_PROJECTS_ITEM_SIX = '';

//services.php
const TXT_SERVICES_TITLE = '';
const TXT_SERVICES_DESC = '';
const IMG_SERVICES_PROYECT_ONE = '';
const TXT_SERVICES_PROYECT_ONE = '';
const IMG_SERVICES_PROYECT_TWO = '';
const TXT_SERVICES_PROYECT_TWO = '';
const IMG_SERVICES_PROYECT_THREE = '';
const TXT_SERVICES_PROYECT_THREE = '';
const IMG_SERVICES_PROYECT_FOUR = '';
const TXT_SERVICES_PROYECT_FOUR = '';

// subscribe.php
const TXT_SUBSCRIBE_TITLE = '';
const TXT_SUBSCRIBE_DESC = '';
const SELECT_SUBSCRIBE_ONE = '';
const SELECT_SUBSCRIBE_TWO = '';
const SELECT_SUBSCRIBE_THREE = '';
const TXT_SUBSCRIBE_BUTTON = '';
